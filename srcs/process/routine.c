...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/28 15:17:38 by mathonon          #+#    #+#             */
/*   Updated: 2023/09/28 15:26:30 by mathonon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_routine(void *arg)
{
	...
	while (philosopher->container->is_plenty == false
		&& philosopher->container->is_dead == false)
	{
		ft_atomic_print(philosopher, THINKING);
		ft_eat(philosopher);
...
static void	ft_eat(t_thread_args *philosopher)
{
	...
	if (philosopher->container->n_meal_till_full != NO_MEAL
		...
		< philosopher->container->n_meal_till_full)
	{
		++philosopher->node->n_meal;
		if (philosopher->node->n_meal
			== philosopher->container->n_meal_till_full)
