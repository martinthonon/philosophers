...
bool ft_process(t_dllist *container)
{
    ...
    if (ft_thread_create(container->size, &philosopher) == true)
        return (true);
    if (ft_mutex_flag(INIT, "%m, %m, %M", &container->lock_print, container->cool_down, container->n_fork, container->size) == true)
        return (free(philosopher), true);
    if (ft_thread_start(container, philosopher, container->sentinel_node->next) == true)
        return (free(philosopher), true);
    if (ft_mutex_flag(DESTROY, "%m, %m, %M", &container->lock_print, container->cool_down, container->n_fork, container->size) == true)
         return (free(philosopher), true);
    free(philosopher);
    return (false);
}
